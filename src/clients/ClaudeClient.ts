...
export class ClaudeClient {
  ...
  async createPlan(task: string, context: ProjectContext, signal?: AbortSignal): Promise<ImplementationPlan> {
    const text = await this.request(PLAN_SYSTEM_PROMPT, buildPlanUserPrompt(task, context), 16000, signal);
  ...
  private async request(system: string, user: string, maxTokens: number, signal?: AbortSignal): Promise<string> {
